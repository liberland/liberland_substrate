...
use crate::{
	mock::*, Config, Electionlock, ElectionlockDuration, Error, Event, LLMAccount, LLMPolitics,
	LastRelease, RemarkData, Withdrawlock, WithdrawlockDuration,
};
use codec::Compact;
use frame_support::{
...
fn only_approved_accounts_can_call_force_transfer() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(LLM::force_transfer(unapproved, from.clone(), to.clone(), amount), BadOrigin);
...
fn cant_force_transfer_more_than_balance() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(LLM::force_transfer(root, from, to, amount), Error::<Test>::LowBalance);
	});
}
...
fn cant_force_transfer_from_liquid() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(LLM::force_transfer(root, from, to, amount), Error::<Test>::InvalidAccount);
	});
}
...
fn force_updates_balances_correctly_to_liquid() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(LLM::force_transfer(root, from, to, amount));
