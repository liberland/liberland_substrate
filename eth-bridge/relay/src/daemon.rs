...
async fn init_sub_tx_manager(
	...
	let arc_tx_manager = Arc::new(tx_managers::Substrate::new(sub_api).await?);
...
pub async fn run(cli: cmdline::Args) -> Result<()> {
	...
	let (sub_tx_manager_handle, sub_tx_manager) = init_sub_tx_manager(sub_api.clone()).await?;
	tasks.push(sub_tx_manager_handle);
