...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn propose(
			...
			ensure!(!testo::check_pooled_llm::<T>(who.clone()), Error::<T>::NoPolLLM);
		...
		pub fn note_preimage(origin: OriginFor<T>, encoded_proposal: Vec<u8>) -> DispatchResult {
			...
			ensure!(testo::check_pooled_llm::<T>(who), Error::<T>::NoPolLLM);
...
impl<T: Config + frame_system::Config<AccountId = T>> Pallet<T> {
	...
	//		use crate::testo::LLMPoliticsCopy;
	//let imb = <T as pallet::Config>::Currency::withdraw();
	//let _T = <T as LLM_Pallet>::has_llm_politics(account);
...
pub mod testo {
	use super::*;
	//use frame_support::traits::StorageInstance;
	//use frame_support::{Twox64Concat, Blake2_128Concat};
	//use LLM_Pallet::AccountId;
	...
	//	pub type StakingValidatorState<T: Config> = dyn StorageMap<
}
