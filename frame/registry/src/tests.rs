...
fn unregister_deposits_event() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Registry::register_entity(registrar, 0, 1, HashingOf::<Test>::hash_of(&data)));
		assert_ok!(Registry::unregister(entity.clone(), 0));
		System::assert_last_event(Event::<Test>::EntityUnregistered { entity: 1, registrar_index: 0 }.into());
	})
}
...
fn request_registration_deposits_event() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Registry::set_entity(entity.clone(), data.clone()));
		assert_ok!(Registry::request_registration(entity.clone(), 0));
		System::assert_last_event(Event::<Test>::RegistrationRequested { entity: 1, registrar_index: 0 }.into());
	})
}
...
fn register_entity_deposits_event() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Registry::request_registration(entity.clone(), 0));
		assert_ok!(Registry::register_entity(registrar, 0, 1, HashingOf::<Test>::hash_of(&data)));
		System::assert_last_event(Event::<Test>::EntityRegistered { entity: 1, registrar_index: 0 }.into());
	})
}
...
fn refund_deposits_event() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(Registry::request_registration(entity.clone(), 0));
		assert_ok!(Registry::refund(entity.clone(), 0));
		System::assert_last_event(Event::<Test>::RefundProcessed { entity: 1, registrar_index: 0 }.into());
	})
}
...
fn instances_dont_mix() {
	new_test_ext().execute_with(|| {
		...
		assert_ok!(SecondRegistry::set_entity(entity.clone(), data.clone()));
		assert_ok!(SecondRegistry::request_registration(entity.clone(), 0));
		assert_ok!(SecondRegistry::register_entity(registrar, 0, 1, HashingOf::<Test>::hash_of(&data)));
